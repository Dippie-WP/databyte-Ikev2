...
history = [
    # 2026-06-25 — v1.6.0
    ("2026-06-25 10:00", "BUGFIX (audit by Zun): one-liner broken in Windows PowerShell 5.1",
     "Misha",
     "Zun: did you even dry run and audit your work look at this shit — PS C:\\WINDOWS\\system32> iex (irm ...setup-databyte-vpn.ps1?slug=safwaan&token=ABC) ParserError AmpersandNotAllowed. Caught me lying again — claimed Windows installer one-liner works, never tested in actual Windows PowerShell 5.1. Two bugs in one: (1) URL had & which PS5.1 parses as background-job operator even in single quotes; (2) MyInvocation.MyCommand.Definition doesnt reliably carry URL when invoked via iex(irm), so URL-detect regex misses anyway. Fixed both: pack slug+token as base64 ?t=BASE64 (no & in URL), change installer_tokens.py to return canonical 3-line block (curl + & + rasdial) per DAT-VPN-WINDOWS-CLIENT-MASTER-001.md, update setup-databyte-vpn.ps1 with Decode-PackedToken helper + 3-way precedence (-t flag > positional args > MyInvocation detect). Also discovered: setup-databyte-vpn.ps1 was NEVER DEPLOYED to /opt/vpn-portal/www/static/ — the customer URL returned 404 even before my changes. Manually scp-d to nginx-served path with correct perms. Live verified: UI renders 3 lines in pre.vp-cmd, Copy button copies full block, no & in URL, PowerShell parser accepts all 3 lines as separate commands.",
     "Critical", "Lesson: NEVER claim a customer-facing flow is shipped without doing the actual end-to-end test in the actual target environment. I tested: (a) server returns JSON, (b) UI shows fields, (c) token was created. I did NOT test: (d) the actual command a customer pastes into Windows PowerShell 5.1 actually runs. Always run the final-step user action through the real shell/UI before claiming done. PowerShell 5.1 (default Windows 10) parses & in command-line URLs even inside single quotes — NEVER use & in PowerShell one-liner URLs."),
