...
roadmap = [
    ...
    ("5D",  "RADIUS migration (Phase 5)",      "FreeRADIUS + daloRADIUS, single MariaDB, portal keeps management. Direct-to-prod, nuke + start fresh (per Zun #23766 + #23783).", "✅ Phase 5 LIVE 2026-07-06 — cut test verified (commit a9d2527). Live 2026-07-13: 5 customers registered, 4 devices active (verified `SELECT COUNT(*) FROM radius.customers` / `devices` / `users`). Previous '40 customers LOCKED OUT' framing was stale assumption — actual count after cut was 5, not 40.", "Phases 1-4 ✅ done (4A SQLAlchemy, 4D reset-restore, 4E MariaDB unification), Phase 5 ✅ live, Phase 7 ✅ cleanups, v2.2.0 ✅ (FreeRADIUS overlay)"),
    ("4E",  "Phase 4E — unify SQLite + MariaDB (post-Phase 5)", "Portal business data (10 tables: customers, users, devices, installer_tokens, tiers, alerts, purchases, audit_log, operator_sessions, customer_portal_sessions) migrated from charon-local SQLite (/var/lib/strongswan/ipsec.db) into MariaDB `radius` DB. SQLite retains only charon-internal tables (addresses, ike_sas, pools, child_configs, certificates).", "✅ SHIPPED 2026-07-12 — commit cb9bf69. Live counters verified 2026-07-13: 5 customers, 4 users, 4 devices, 1 operator, 333 audit_log, 114 installer_tokens, 8 tiers, 2 alerts. See docs/PHASE-4E-DEPLOYMENT-NOTES.md and docs/PHASE-4E-EXECUTION-PLAN.md. docs/PHASE-4E-SCOPE-ASSESSMENT.md marked SUPERSEDED.", "—"),
    ("7.5", "Phase 7.5 — iptables-legacy → nftables swap", "Per-VIP quota counters migrated to nftables named counters in `inet quota_table`. Persistent across `nft flush ruleset`; eliminates 5B.6 bug class permanently. VPS only; LXC 903 lab retains iptables-legacy.", "✅ LIVE on VPS 2026-07-09 — per TOOLS.md vps-01 `nft list ruleset | grep quota` returns `table inet quota_table`. 2 iptables services likely still present for legacy compat but are not authoritative.", "—"),
    ("v2.2", "v2.2.0 — FreeRADIUS operator overlay + radacct chain", "host/freeradius/ + provision-freeradius.sh captures sql_last_seen + queries.conf + sites-enabled/default. Idempotent (md5-gated copy + restart only on change) + smoke test (Accounting-Request via radclient) + --check for CI (exit 1 on drift). Radacct chain end-to-end verified on real client traffic.", "✅ SHIPPED 2026-07-13 12:35 SAST — commit 805ea84, tag v2.2.0 (annotated, → 805ea84^{commit}). Live 5 radacct rows + 390 radpostauth. Idempotency proven (clean / tamper / restore). See CHANGELOG.md v2.2.0.", "—"),
...
changes = [
    # --- 2026-07-13 — v2.2.0 + doc-audit (this commit) ---
    ("2026-07-13 16:00", "v2.2.0 doc-audit: ARCHITECTURE / ROADMAP / DEPLOYMENT / VPS-XNEELO-DEPLOY / PHASE-4E-SCOPE-ASSESSMENT synchronized with live state",
     "User Zun 2026-07-13 11:10 UTC: 'is all our runbooks and documents architecture only update'. Drift audit found: ARCHITECTURE.md claimed 'SQLite (NOT Postgres, NOT MySQL)' + 'iptables-legacy (NOT nftables)' (both STALE post-Phase 4E / Phase 7.5); ROADMAP.md still listed 5D as 🟡 In progress (Phase 5 LIVE for 1+ week); DEPLOYMENT.md had no FreeRADIUS install step + referenced a non-existent rollback script; VPS-XNEELO-DEPLOY.md said 'DB: SQLite' + 'iptables-legacy firewall' + 'portal v1.9.0' + 'RustFS daily backup' (all stale by 1+ week); PHASE-4E-SCOPE-ASSESSMENT.md was SUPERSEDED by EXECUTION-PLAN + DEPLOYMENT-NOTES but not marked. Updated each in-place with '🟢 Verified-live 2026-07-13' tables citing SSH + mariadb + git evidence; flagged stale prose as STALE rather than rewriting (preserves historical record). All receipts are tool-verified, not claimed.",
     "commit (this docs-sync), REGENERATED tracker/databyte-vpn-tracker.xlsx; 5 doc files modified in-place: ARCHITECTURE.md, ROADMAP.md, DEPLOYMENT.md, VPS-XNEELO-DEPLOY.md, PHASE-4E-SCOPE-ASSESSMENT.md",
     "🟢 Low", "rustfs:/open-claw-push/docs-audit-2026-07-13/ for user review (per Zun msg 'Push to Rustfs docs to review'). GitHub commit + push pending Zun approval.",
     "✅"),
    ("2026-07-13 12:35", "v2.2.0 release — FreeRADIUS operator overlay + radacct chain wired end-to-end",
     "Five of the six CORR-035 fixes for radacct/last_seen had been applied directly to /etc/freeradius/3.0/ on vps-01 but never backported to the repo. v2.2.0 closes the gap: host/freeradius/ overlay captures sql_last_seen (sql_user_name), queries.conf (22x MariaDB-compat template), sites-enabled/default (line 735 -sql→sql). provision-freeradius.sh is idempotent (md5-gated copy + restart only on change + smoke test) with --check returning exit 1 for CI.",
     "origin/main commit 805ea84, tag v2.2.0 → 0188a7e4 (annotated) → commit 805ea84. Files: host/freeradius/{README.md, provision-freeradius.sh, mods-available/sql_last_seen, mods-config/sql/main/mysql/queries.conf, sites-enabled/default}.",
     "🟠 High", "Live radacctid=2 (Windows, 5.0MB out) + radacctid=3 (iPhone, 26MB in / 29MB out) with 300s interim updates. devices.id=104.last_seen_v4 = 105.174.128.86 (clean, no RFC6733 brackets). provision-freeradius.sh exits 0 on clean, exits 1 on drift. GitHub Release v2.2.0 body 4101 chars (PATCH /releases/353095019).",
     "✅"),
    ("2026-07-12 08:17", "Phase 4E — unified portal SQLite → MariaDB (commit cb9bf69)",
     "Portal business data (customers, users, devices, installer_tokens, tiers, alerts, purchases, audit_log, operator_sessions, customer_portal_sessions) moved from /var/lib/strongswan/ipsec.db SQLite into MariaDB `radius` DB. SQLite retains only charon-internal tables (addresses, ike_sas, pools, child_configs, certificates). 10 tables migrated, all counts match + 4/4 NTLM hex strings byte-identical. Single source of truth for portal state.",
     "commit cb9bf69 (origin/main), docs/PHASE-4E-{DEPLOYMENT-NOTES,EXECUTION-PLAN}.md, scripts/migrate_sqlite_to_mariadb.py. CORR-2026-07-12-028 logged.",
     "🔴 Critical", "Live: 162 tests pass; portal /api/health → {db_ok:true, db_customers:5, charon_ok:true}; radpostauth grew 323→324 with new MSCHAPv2 auth using migrated NTLM hashes (proves BLOB roundtrip is byte-perfect).",
     "✅"),
    ("2026-07-11 21:00", "v2.1.0 — case-insensitive identity normalization (CORR-026) + drift-detect CI",
     "Three sister files all did WHERE u.name = ? against users.name (charon SQLite, case-SENSITIVE) while FreeRADIUS used utf8_general_ci (case-INSENSITIVE). Auth succeeded at FreeRADIUS but downstream lookups returned 0 rows: quota-monitor missed customers, bandwidth-monitor applied wrong tier, login rejected.",
     "commits: 3 sites fixed in portal_auth.py + installer_tokens.py + bulk_action.py. CI: .github/workflows/drift-detect.yml (calls check-portal-deployed.sh against VPS, catches main↔VPS SHA divergence).",
     "🟠 High", "Live iperf3 + ifconfig.me + portal login flow with mixed-case identities (SIRAJ-iPhone, siRaj-Iphone, etc.) — all work. /api/health green. drift-detect workflow PASS on tag v2.1.0.",
     "✅"),
    ("2026-07-06 16:45", "v2.0.0 release — Phase 5 eap-radius cutover + DAE + reset bug fix + Phase 7 cleanups",
     "First v2 baseline. System at HEAD is no longer the same architecture as v1.7.0-recovered (01a475f): customer EAP identities live in MariaDB radcheck/radusergroup, not in rw-eap.conf. Quota enforcement cuts by killing radcheck + sending a signed RFC 5176 Disconnect-Request to charon eap-radius.dae, not by rewriting the local EAP secret. Reset flow restores radcheck from rw-eap.conf.bak-quotamon-<epoch>. Phase 7 cleanup 1 (vestigial eap-radius blocks removed from rw-eap.conf) + cleanup 3 (FreeRADIUS IPv6 secret realigned — root cause of 15+ 'Invalid Message-Authenticator' bursts).",
     "commits a9d2527 (CHANGELOG.md v2.0.0 entry + app.js SCRIPT_VERSION 'v1.9.0-sse' → 'v2.0.0' + app.py FastAPI version '0.1.0' → '2.0.0') + 8b662d9 (systemd vpn-portal.service Description → 'v2.0.0, 2026-07-06'). Annotated git tag v2.0.0 on a9d2527.",
     "🟠 High", "Deployed to vpn-prod-01 2026-07-06 16:45 UTC via deploy-portal-vps.sh 'v2.0.0' marker. STEP 8.5 customer-facing flow smoke test 13/13 PASS, 0 FAIL, 0 WARN. /api/health → 200 OK, db_customers=6, charon_ok=true. Live portal SCRIPT_VERSION=v2.0.0 (curl app.js?v=8b662d9). Live systemctl show vpn-portal.service Description=v2.0.0 (was stale v1.3.1 for 2+ weeks — closed deployment gap manually: sudo cp /opt/vpn-portal/systemd/vpn-portal.service /etc/systemd/system/ + systemctl daemon-reload). check_github_parity.sh PASS post-push. portal-smoke GHA #229 success on main @ a9d25272. release.yml GHA #36 (v2.0.0 tag push) builds GHCR image + auto-creates GitHub Release.", "✅"),
    ...
    ("2026-06-25 19:49", "quota-monitor poll 60s → 10s (perf fix)",
     "At 60s poll interval, zade hit 100% and overran by ~34 MB before next poll detected it. 10s reduces max overrun to ~6-12 MB at 40Mbps cap.",
     "quota/quota-monitor.py (commit 8d63d4b) — POLL_INTERVAL = 10",
     "🟢 Low (perf)", "Deployed. Boot log says 'poll every 10s'. Iteration gap verified 10s exact. CPU: 0.2%, Memory: 0.3%. Zun directive msg #22356.", "✅"),
    ("2026-06-25 19:22", "Portal v1.6.7 — fix reset_quota KILLED-secret restore",
     "Hard cut at 100% KILLs customer's EAP secret. But Reset usage button didn't restore it — dev_names built from device_name ('cellphone') but rw-eap.conf uses EAP identity ('zade-cellphone'). State machine missed every cut. Caught by Zun: 'i reset zade data usage after he reach the hard cut but he was still unable to connect back'.",
     "host/vpn-portal/app.py (commit 0ba4e69) — JOIN devices→users for eap_identity; tests/test_reset_quota_secret_restore.py (3 new tests)",
     "🔴 High", "Deployed. Manual restore on zade: POST /api/quota/60/reset returned secret_restored:true, devices:[zade-cellphone]. Conf shows secret=XqGQiqkQJ00TT5zKARXkSw (was KILLED-9ef53187e3d7216f). 127 tests passing (was 124 + 3 new).", "✅"),
...
bugs = [
    ...
    ("2026-07-05 22:38", "clients.conf secret diagnostic (radtest → invalid Message-Authenticator)",
     ...
     "RESOLVED 2026-07-06 00:33 UTC: was USER ERROR in last-session radtest invocation, not a config bug. clients.conf has TWO localhost blocks with DIFFERENT secrets (client localhost IPv4 = 9f32...a0ebd2; client localhost_ipv6 = testing123 default). My last-session command passed `testing123` but hit the IPv4 block. Verified correct flow with radtest ... 127.0.0.1 0 9f32...a0ebd2 → Access-Accept.",
     "🟠 High", "✅ RESOLVED 2026-07-06 00:33 UTC — was user error, not config bug (see MEMORY.md RADIUS section).", "TODO RADIUS-Phase-5"),
    ...
    # 2026-07-06 — quota cut
    ("2026-07-06 06:18", "Quota cut doesn't actually lock customer out (Phase 5 cutover)",
     "End-to-end cut test on customer 86 (zun-iphone-test, 100MB cap) fired 18 cut_100pct attempts in 9 min, ALL FAILED. Reason: Phase 5 cutover changed charon auth from local rw-eap.conf secrets to RADIUS (`auth = eap-radius`), but quota-monitor _cut_customer() still only kills rw-eap.conf secrets. For Phase 5+ customers there's NO rw-eap.conf block → cut returns False → no cut. For legacy customers the rw-eap.conf kill does nothing because charon now asks RADIUS. RADIUS still has Cleartext-Password → Access-Accept → reconnect.",
     "Added disable_customer_radcheck() to quota/quota-monitor.py — mirrors portal_auth.disable_customer_radcheck(). DELETEs radcheck rows + INSERTs Cleartext-Password := DISABLED-<random> marker. _cut_customer() now calls radcheck disable FIRST (PRIMARY), rw-eap.conf kill SECOND (defense-in-depth). Reads /root/.mariadb-radius-pw via MYSQL_PWD env var. Commit 649918f. Phase 7 attribution synced in commit 9a93832.",
     "🟠 High", "✅ FIXED 2026-07-06 06:18 UTC — commit 649918f + 9a93832. Cut test on customer 86: radcheck DISABLED-4e432132bf3400ab, over_quota=1, customer can't reconnect.", "MEMORY.md Phase 5 cutover"),
    ("2026-07-06 06:25", "scp overwrote Phase 7 pool-lease attribution code (CORR-2026-07-06-002)",
     "scp'd quota-monitor.py from local HEAD to VPS to deploy the radcheck-disable fix. Only checked the kill function I edited — did NOT diff the whole file. Local main was STALE relative to VPS (Phase 7 commit 7a55d15 on branch backup-broken-v1.9.1-pre-reset was deployed-only). My scp overwrote the working Phase 7 (30825 bytes) with my main-branch HEAD + fix (27354 bytes). Without Phase 7, iOS native IKEv2 SA churn breaks quota attribution (the original Phase 5/6 bug Zun caught via msg #22544).",
     "Restored Phase 7 from git (git show backup-broken-v1.9.1-pre-reset:quota/quota-monitor.py), applied radcheck fix on top → 880 lines, deployed via scp, verified live (cut test success). Committed as 9a93832 to sync local repo with VPS so next deploy won't regress. Added cross-check protocol entry: BEFORE scp, diff ENTIRE file with sha256sum, not just the edited function.",
     "🟠 High", "✅ FIXED 2026-07-06 06:25 UTC — Phase 7 + radcheck-fix deployed together (commit 9a93832, on origin/main). 3rd occurrence of 'local artifacts drifted from VPS reality' pattern → HOT rule promoted.", "MEMORY.md cross-check protocol + ~/self-improving/corrections.md CORR-2026-07-06-002"),
]
...
tofix = [
    ...
    ("Quota cut radcheck disable fix (CORR-2026-07-06-002 — scp overwrote Phase 7)",                "✅ Done", "20m", "5D RADIUS migration", "disable_customer_radcheck() added to quota-monitor.py + _cut_customer now calls it FIRST. Phase 7 list-pool-leases attribution synced from backup-broken-v1.9.1-pre-reset branch to main (commit 9a93832). 7 unit tests in tests/test_quota_radcheck_disable.py."),
]
