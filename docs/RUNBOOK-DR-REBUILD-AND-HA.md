...
**9 verified-live gaps** — every row below re-checked by direct command on `ssh root@vps-01` in this turn (post initial audit pass). Items that could not be live-verified or were mis-stated in v1.0.1's first draft are documented in the separate "Defensive notes" subsection below — NOT counted in the 9.
...
| 3 | **MariaDB `radius` DB = 40 tables, NOT just 7 RADIUS-protocol tables.** Live `SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='radius'` returns 40. The 7 RADIUS-protocol tables (`radcheck`, `radreply`, `radgroupcheck`, `radgroupreply`, `radusergroup`, `radpostauth`, `radacct`) are joined by ~33 daloRADIUS admin/operator/billing tables (`operators`, `operators_acl`, `customers`, `users`, `devices`, `billing_*`, `nas`, `hotspots`, `userinfo`, `userbillinfo`, `installer_tokens`, etc.). | `mariadb -uroot radius -e 'SELECT COUNT(*) FROM information_schema.tables WHERE table_schema="radius"'` → 40 | Operator accidentally drops daloRADIUS tables thinking they're portal/legacy → kills admin UI permanently. The portal's `customers` / `devices` / `users` SQL JOINs target the daloRADIUS-named tables (per CORR-022), not separate portal tables. | §2.3 step 14 (rewritten): verify both via `SELECT COUNT(*) FROM radcheck` AND `SELECT COUNT(*) FROM operators` — both must be >0 |
| 4 | **`radacct` is EMPTY** while `radpostauth` has 323 rows. FreeRADIUS **accounting chain is NOT wired**; only **authentication** is. | Live: `mariadb -uroot radius -e 'SELECT COUNT(*) FROM radacct'` → 0; `SELECT COUNT(*) FROM radpostauth` → 323 | RADIUS-derived billing/time-online/session reports impossible. Customer-visible impact: zero today (quota enforcement uses nftables counters in `quota-monitor.py`, NOT RADIUS accounting). | New §2.3 step 14a: `radtest` (accounting-request) check + `radacct` row count acceptance — document but do not regression-fix; current state is the baseline |
...
| 13 | **`/var/log/charon-log-host/charon.log` has NO logrotate** (live `cat /etc/logrotate.d/charon-log-host` → ENOENT; `ls /etc/logrotate.d/ \| grep -i charon` → empty). Current size 16,666,802 B (16 MB), growing monotonically. Over weeks/months this will fill `/var/log` disk → portal outages. **Operational gap, NOT a rebuild-breaker**, but worth fixing while we're touching these paths. | `stat -c "%s" /var/log/charon-log-host/charon.log` (live 16.6 MB); `logrotate -d` would show no schedule for charon log | `/var/log` fills; charon may fail to write silently; portal becomes unreachable | New §2.3 step 11d: write `/etc/logrotate.d/charon-log-host` config (daily, rotate 7, compress, create 0644 root root); `systemctl reload logrotate` |
...
| v1.0.3 | 2026-07-12 08:05 UTC / 10:05 SAST | Misha 🐻 | Deep docker-mount verification (Zun msg #25444, dropped "Customer re-connection" risk from 15% — operationally covered by portal installer flow). **§1.6 extended 9 → 13 verified-live gaps**. New gaps from `docker inspect strongswan` + bind-source verification: (10) `strongswan` GID 1002 group must pre-exist on fresh Debian 13; (11) `/var/log/charon-log-host/` mkdir precondition for bind source; (12) image-baked `/etc/strongswan.d/` vs 4 bind mounts (bind carries customer EAP identities, image-baked does not); (13) `/var/log/charon-log-host/charon.log` has NO logrotate (16 MB and growing → disk fill). **§2.3 step 17 rewritten** with EXACT `docker run` command copied from `docker inspect strongswan` output (7 `-v` binds, `--cap-add CAP_NET_ADMIN CAP_NET_RAW`, `--health-cmd`, `--restart unless-stopped`). New §2.3 steps: 11d (logrotate config install), 16a (`groupadd strongswan` + `/var/lib/strongswan` perms precondition), 16b (`/var/log/charon-log-host/` mkdir precondition). Step 18 removed (rw-eap.conf auto-mounted via step 17 — no separate restore). |
