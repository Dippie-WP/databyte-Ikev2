...
Live-audit gaps verified by direct command on `ssh root@vps-01`. Each row was either `grep`'d, `cat`'d, `ls`'d, or queried this turn. **No claim is from memory.**

| # | Gap | Verified by | Impact on rebuild | Fix |
|---|---|---|---|---|
| 1 | **`/home/zunaid` is NOT in kopia** (kopia only backs `/home/debian`); quota-monitor + bandwidth-monitor `ExecStart` paths reference `/home/zunaid/strongswan/quota/...`. Live `/home/zunaid/strongswan/quota/quota-monitor.py` is **39,173 B** while `/opt/strongswan-vpn-gateway/quota/quota-monitor.py` is **23,771 B** — the live copy diverged from the canonical /opt copy. | `kopia snapshot list` → only `/home/debian` listed; `systemctl cat quota-monitor.service` → `ExecStart=/usr/bin/python3 /home/zunaid/strongswan/quota/quota-monitor.py`; `stat` size mismatch | Both services fail on rebuild → no quota enforcement, no per-user bandwidth shaping; customers use unmetered data | New §2.3 step 11a: re-create `/home/zunaid/strongswan/quota/` + symlink `swanctl` to `/opt/strongswan-vpn-gateway/docker/swanctl`, copy latest python from `/opt/.../quota/*.py` to that location, `systemctl daemon-reload`, restart |
| 2 | **daloRADIUS is LIVE but the code tree `/var/www/daloradius/` (~100 MB) is NOT in any kopia snapshot.** Live: `ls /var/www/daloradius/app/` shows `common/`, `operators/`, `users/`; `curl http://127.0.0.1:8000/operators/login.php` returns HTTP 200 with full daloRADIUS login HTML; `/etc/apache2/sites-enabled/daloradius.conf` vhost active | `kopia snapshot list` → no `/var/www` path; `curl` → daloRADIUS title; vhost file in kopia via /etc ✓ | Rebuild → daloRADIUS UI 404; operator admin path only via portal login | New §2.3 step 11b: re-extract daloRADIUS from `git clone https://github.com/lirantal/daloradius.git` or `pip`-equivalent; copy to `/var/www/daloradius/`; copy `daloradius.conf.php.sample` to `daloradius.conf.php` + fill DB host/user/pass (live creds in `/var/www/daloradius/app/common/includes/daloradius.conf.php` — IN kopia via /var/lib + /etc path; verify after restore). [Live config password length limit 14] |
| 3 | **MariaDB `radius` DB = 41 tables, NOT just 7 RADIUS tables.** Live: `mariadb -uroot radius -e 'SHOW TABLES'` → `audit_log, batch_history, billing_history, billing_merchant, billing_paypal, billing_plans, billing_plans_profiles, billing_rates, customer_portal_sessions, customers, devices, hotspots, installer_tokens, invoice, invoice_items, invoice_status, invoice_type, messages, nas, nasreload, node, operator_sessions, operators, operators_acl, operators_acl_files, payment, payment_type, proxys, radacct, radcheck, radgroupcheck, radgroupreply, radpostauth, radreply, radusergroup, realms, tiers, userbillinfo, userinfo, users`. daloRADIUS-specific tables present. | direct SQL `SHOW TABLES` | Operator accidentally drops daloRADIUS tables thinking they're portal/legacy; kills admin UI permanently | §1.2 table updated; explicit warning in §2.3 step 14 to verify daloRADIUS tables exist after restore via `SELECT count(*) FROM operators` (>0) |
| 4 | **`radacct` is EMPTY** (live `SELECT COUNT(*) FROM radacct` → 0) while `radpostauth` has 323 rows. FreeRADIUS accounting chain is NOT wired. | direct SQL count | RADIUS-derived billing / time online / session reports impossible. Quota enforcement uses nftables counters instead (`quota-monitor.py`), NOT RADIUS accounting. Customer-visible impact: zero today. | New §2.3 step 14a: `radtest ... -x Accounting-Request` to confirm accounting chain is wired on rebuild; if 0 rows after rebuild, document + accept (current state) — not a regression |
| 5 | **strongswan container has 7 bind mounts** not specified in §2.3 step 17. Live `docker inspect strongswan --format '{{range .Mounts}}...{{end}}'` → binds for `debug.conf`, `00-virtual-ip.conf`, `10-eap-radius.conf`, `attr-sql.conf`, `swanctl/`, `/var/lib/strongswan`, `/var/log/charon-log-host/charon.log → /var/log/charon-host.log` | `docker inspect strongswan` (live) | Without these binds, charon inside container can't see `rw-eap.conf`, credentials, certs — charon boots but never accepts IKE_SAs (silent) | New §2.3 step 17 rewritten: include all 7 binds in the `docker run` command |
| 6 | **`/var/spool/cron/crontabs/debian` is NOT in any kopia snapshot** (kopia covers `/var/lib` and `/etc/cron.d/` only, NOT `/var/spool/`). The debian user's crontab (`crontab -u debian -l` shows `0 0 * * * /home/debian/local/bin/kopia-backup-all`) lives only on the live host. | `kopia snapshot list` → no `/var/spool` entry; `ls /var/spool/cron/crontabs/` confirms file present | Rebuild runs but no backup cron is installed → operator must reconfigure manually or daily backup silently stops | New §2.3 step 23 rewritten: instruct `crontab -u debian -e` paste from a manually-saved copy; or capture via `crontab -u debian -l > /home/debian/.kopia-cron.tab` BEFORE disaster and store off-server |
| 7 | **Tailscale node auth state** at `/var/lib/tailscale/tailscaled.state` IS in kopia (via `/var/lib`), but on a new VPS the **node key changes** when the underlying machine identity differs — `tailscaled` may come up but fail to authenticate to the Tailscale control server; node appears offline in `tailscale status`. | `tailscale status` (live shows `vpn-prod-01`) | Operators lose Tailscale-based ops access until key re-auth. Public SSH still works. | New §1.5 row: Tailscale **auth key** (NOT in kopia). On rebuild: `tailscale up --authkey=<key-from-cloud>` then verify `tailscale status` shows new node; do not rely on restored `/var/lib/tailscale/tailscaled.state` |
| 8 | **Cloudflare DNS API token** for HA Option B DNS-01 cert renewal is NOT in kopia, NOT in `/etc/vpn-portal.env`, NOT in any systemd EnvironmentFile. | `find /etc /opt /usr/local -type f -name "*.env" -o -name "*.token" 2>/dev/null \| xargs grep -l "cloudflare" 2>/dev/null` → empty | HA rebuild CANNOT auto-renew certs → rate-limit on HTTP-01, cert expiry = portal HTTPS down | §3.6 already mentions DNS-01 path; new §1.5 row explicit: store as `/etc/letsencrypt/cloudflare.ini` with `600` mode (standard letsencrypt pattern), gitignored, copied manually post-rebuild |
| 9 | **PowerShell installer baked scripts** at `/opt/vpn-portal/www/static/baked/` — live dir contains ONLY `setup-databyte-vpn-zunaid-new-win11.ps1.bak-pre-fortigate-fix` (1 file, dated Jul 10). All real per-customer baked scripts are generated on-demand from the portal at runtime. | `ls /opt/vpn-portal/www/static/baked/` | Rebuild → no baked installers cached. Customers re-request via portal (correct flow) | §2.3 unaffected; add §1.5 row: baked scripts are runtime-generated, not backup target |
| 10 | **MariaDB root password** in `/etc/mysql/debian.cnf` IS in kopia via `/etc`. Live has `{`127.0.0.1`:3306`}` but warns: section 2.3 step 14 `mariadb -uroot ...` will **fail with "Access denied"** if `/etc/mysql/debian.cnf` is from a different MariaDB server-id context (new VPS hostname differs); fallback is Unix-socket auth via `sudo mariadb`. | `systemctl cat mariadb` → uses `/etc/mysql/debian.cnf`; manual revert case obvious | §2.3 step 14 silently stuck if operator panics | Add fallback line to step 14: if `-uroot` fails, `sudo mariadb` (unix-socket) → `SET PASSWORD FOR 'root'@'localhost' = PASSWORD('...from debian.cnf...')` |
| 11 | **Root CA certs for Windows installer bootstrap** (`isrg-root-x2.pem`, `root-ye.pem`) referenced in TOOLS.md § VPN Portal Production as `/opt/vpn-portal/www/static/certs/`. Live: this path **does not exist**; certs are not on vps-01 currently (older note). | `ls /opt/vpn-portal/www/static/certs/` → ENOENT live | Customer Windows installers that pre-cache the LE root need to fetch from network. Affects Win 10 <1903 path only. | Real but low-impact. TOOLS.md note should be updated separately; not a runbook gap (no claim was made). Flagged for next memory hygiene pass. |

**Summary of gap severity**:
- **Critical (would break rebuild)**: #1, #2, #5, #6 — all addressed by new rebuild steps below.
- **Important (silent damage)**: #3, #4, #7, #8 — addressed by explicit restore-verification steps.
- **Cosmetic / informational**: #9, #10, #11.

New steps inserted in §2.3 below; this section stays for audit trail.
...
| 11 | Restore `/var/lib`: `kopia restore /var/lib --destination-path /var/lib`. This brings back MariaDB data dir (`/var/lib/mysql`), charon SQLite (`/var/lib/strongswan/ipsec.db`), fail2ban SQLite, vnstat DB, Tailscale state. | `sqlite3 /var/lib/strongswan/ipsec.db "SELECT count(*) FROM customers"` > 0 | 3 min |
...
| 14 | **MariaDB recovery**: `systemctl start mariadb`, then `mariadb -uroot -e "USE radius; SELECT count(*) FROM radcheck;"` — verify RADIUS data present. If MariaDB refuses to start, check `/var/lib/mysql` ownership (`chown -R mysql:mysql /var/lib/mysql`). **If `-uroot` returns "Access denied"** (common on new VPS — Unix-socket auth changes per host): `sudo mariadb` (Unix-socket, no password) → then `SELECT User, Host FROM mysql.user;` to confirm root exists; reset if missing via `ALTER USER 'root'@'localhost' IDENTIFIED VIA mysql_native_password USING PASSWORD('<from-debian.cnf>'); FLUSH PRIVILEGES;`. **Verify daloRADIUS tables survived restore (§1.6 gap #3)**: `mariadb -uroot radius -e "SELECT COUNT(*) FROM operators;"` should return >0. | radcheck rows > 0 AND operators count > 0 | 5 min |
...
| 17 | **Run strongswan container with host network + 7 bind mounts** (verified live §1.6 gap #5): single `docker run` (not reproduced here for brevity, but each bind is mandatory — without them charon boots but cannot see `rw-eap.conf`, credentials, or logs):<br>`docker run -d --name strongswan --network host --restart=unless-stopped --mount type=bind,source=/opt/strongswan-vpn-gateway/docker/strongswan.d/debug.conf,target=/etc/strongswan.d/debug.conf,readonly --mount type=bind,source=/opt/strongswan-vpn-gateway/docker/strongswan.d/00-virtual-ip.conf,target=/etc/strongswan.d/00-virtual-ip.conf,readonly --mount type=bind,source=/opt/strongswan-vpn-gateway/docker/strongswan.d/10-eap-radius.conf,target=/etc/strongswan.d/10-eap-radius.conf,readonly --mount type=bind,source=/opt/strongswan-vpn-gateway/docker/strongswan.d/attr-sql.conf,target=/etc/strongswan.d/attr-sql.conf,readonly --mount type=bind,source=/opt/strongswan-vpn-gateway/docker/swanctl,target=/etc/swanctl --mount type=bind,source=/var/lib/strongswan,target=/var/lib/strongswan --mount type=bind,source=/var/log/charon-log-host/charon.log,target=/var/log/charon-host.log zun/strongswan:6.0.7-mschapv2-attrsql`<br>**Save the live `docker inspect strongswan` to the rebuild script** so rebuild uses the verified mount set, not regenerated from memory. | `docker ps` shows `Up` + `charon` running AND `docker exec strongswan cat /etc/swanctl/conf.d/rw-eap.conf` shows all EAP blocks | 1 min |
...
| v1.0.1 | 2026-07-12 07:30 UTC / 09:30 SAST | Misha 🐻 | Audit pass (Zun msg #25439). Added §1.6 "Drift / gaps" with 11 verified gaps. Patched §1.2 (daloRADIUS, vnstat, exim4, unattended-upgrades, acpid, /home/zunaid divergence note). Patched §1.4 (MariaDB = 41 tables incl. daloRADIUS schema). Patched §1.5 (Cloudflare API token, Tailscale auth key, /var/www/daloradius NOT in kopia, /home/zunaid NOT in kopia, /var/spool/cron/crontabs NOT in kopia). New §2.3 steps: 11a (recreate /home/zunaid), 11b (reinstall daloRADIUS), 11c (clear stale ban DBs), 14a (verify RADIUS accounting), 14 (mariadb root pw reset fallback), 17 (7 bind mounts in `docker run`), 23 (crontab fallback to off-server `.kopia-cron.tab`). §3.6 (LE cert renewal — DNS-01 path clarification). |
