...
| `freeradius.service` | AAA backend (RADIUS UDP 1812/1813); strongSwan → eap-radius → FreeRADIUS → MariaDB `radius` DB | config in `/etc/freeradius/` (snapshotted); SQL module enabled, **auth in `radpostauth` (327 rows live) AND `radacct` (3 rows live)** — operator overlay `host/freeradius/` applies 3 critical config files (sql_last_seen, queries.conf, sites-enabled/default) + chown radacct directory |
...
| 4 | **~~`radacct` is EMPTY~~ RESOLVED in v2.2.0 (CORR-2026-07-13-035, 2026-07-13 12:35 SAST):** FreeRADIUS accounting chain is now wired. Live `radacct` has 3 rows (radacctid=1 smoke test, radacctid=2 zunaid-win11-en-laptop, radacctid=3 zun-iphone). Interim updates fire every 300s with real octet counters. Six bugs fixed in one session: (1) `sql_last_seen` missing `sql_user_name` directive, (2) charon `accounting = no` upstream default, (3) charon `station_id_with_port = yes` upstream default (caused `=5B...=5D` quote-encoded IPs), (4) `sites-enabled/default` line 735 `-sql` (disabled), (5) `/var/log/freeradius/radacct/` owned by `root:root`, (6) `queries.conf` SQLite-flavored `${....event_timestamp}` template. All 6 fixes captured in `host/freeradius/` overlay + `provision-freeradius.sh` (idempotent, with smoke test). | `mariadb -uroot radius -e 'SELECT COUNT(*) FROM radacct'` → 3+; `bash /opt/strongswan-vpn-gateway/host/freeradius/provision-freeradius.sh --check` → exit 0 | If `radacct` becomes empty AGAIN post-rebuild, the operator overlay was not applied — run `provision-freeradius.sh`. If overlay runs but `radacct` still empty, check strongswan eap-radius `accounting = yes` (in `docker/strongswan.d/10-eap-radius.conf`). | §2.3 step 14a (rewritten in v2.2.0): apply `provision-freeradius.sh` + verify `radacct` ≥1 within 5 min of first client connect. |
...
| 14a | **Apply FreeRADIUS operator overlay** (§1.6 gap #4 — `radacct` MUST have rows post-rebuild; if empty, the build is broken even if auth works):<br>`bash /opt/strongswan-vpn-gateway/host/freeradius/provision-freeradius.sh`<br>This idempotently applies the 3 critical config files (sql_last_seen, queries.conf, sites-enabled/default), fixes `/var/log/freeradius/radacct/` ownership, restarts FreeRADIUS, and runs a smoke test (sends Accounting-Request via `radclient`, expects Accounting-Response). Exit 0 = OK; non-zero = investigate.<br>**Verify post-rebuild:** `mariadb -uroot radius -e "SELECT COUNT(*) FROM radacct;"` must be ≥1 within 5 min of first client connect (radacctid=1 is the smoke test row, radacctid≥2 is real client traffic).<br>See `host/freeradius/README.md` for full operator overlay design. | provision exits 0 AND radacct ≥1 | 2 min |
